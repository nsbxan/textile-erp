...
export function Navbar({ onOpenNewRoll }) {
  ...
  return (
    <>
      <header className="h-16 bg-white dark:bg-[#0f172a] border-b border-slate-200 dark:border-slate-800 px-3 sm:px-4 md:px-6 flex items-center justify-between z-30 shrink-0 select-none transition-colors duration-200">
        {/* Chap tomon: Mobil Menu Button + Brand va sarlavha */}
        <div className="flex items-center gap-2 sm:gap-3">
          {/* Mobil Menyu Ochish Tugmasi (faqat kichik ekranlarda) */}
          <button
            type="button"
            onClick={() => setMobileMenuOpen(prev => !prev)}
            className="p-2 -ml-1 rounded-xl text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 lg:hidden cursor-pointer"
            title="Bo'limlar menyusi"
          >
            <Menu className="w-5 h-5 text-teal-600 dark:text-teal-400" />
          </button>

          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-tr from-teal-600 to-emerald-500 flex items-center justify-center text-white font-extrabold shadow-sm shrink-0">
            <Building className="w-4 h-4 sm:w-5 sm:h-5" />
          </div>
          <div>
            <h1 className="text-sm sm:text-base md:text-lg font-black tracking-tight text-slate-900 dark:text-white leading-tight truncate max-w-[130px] sm:max-w-xs md:max-w-none">
              {settings.brandName || "TextilePro Uzbekistan"}
            </h1>
            <p className="text-[10px] sm:text-[11px] font-semibold text-slate-500 dark:text-slate-400 hidden sm:block">
              {lang === 'ru'
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3">
          {/* Bugungi Sana va Jonli Soat (sekundlari bilan) - Katta ekranlarda ko'rinadi */}
          <div
            className="hidden xl:flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 rounded-xl bg-slate-100/90 dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 text-xs shadow-xs select-none"
          ...
          <button
            ...
            className="flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 text-emerald-800 dark:text-emerald-300 text-xs font-bold hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-all cursor-pointer shadow-xs"
          >
            <DollarSign className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400 shrink-0" />
            <span className="hidden sm:inline">1$ = {Number(usdRate).toLocaleString()} {lang === 'ru' ? 'сум' : lang === 'cyr' ? 'сўм' : "so'm"}</span>
            <span className="sm:hidden font-mono">{Number(usdRate).toLocaleString()}</span>
          </button>
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-xs font-extrabold hover:bg-slate-200 dark:hover:bg-slate-700 transition-all cursor-pointer shadow-xs"
            ...
            >
              ...
              <span className="sm:hidden uppercase text-[11px] font-bold">{lang}</span>
